...
interface ImportLoansModalProps {
    isOpen: boolean;
    onClose: () => void;
    onImport: (data: Record<string, any>[]) => void;
}

const REQUIRED_HEADERS = ['nome', 'valor_emprestado', 'taxa_juros', 'dia_vencimento'];
const TEMPLATE_CSV = `"NOME","VALOR EMPRESTADO","TAXA JUROS %","DIA VENCIMENTO","STATUS"\n"Maria Silva","1500,00","20","15","Aguardando"\n"João Costa","2000,00","15","7","Atrasado"`;


const ImportLoansModal: React.FC<ImportLoansModalProps> = ({ isOpen, onClose, onImport }) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                if (missingHeaders.length > 0) {
                    setError(`O arquivo não contém as colunas obrigatórias: ${missingHeaders.join(', ')}. Verifique o template.`);
                    return;
                }
    ...
    const handleDownloadTemplate = () => {
        ...
        link.href = url;
        link.setAttribute('download', 'template_importacao_emprestimos.csv');
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleClose} title="Importar Planilha de Empréstimos">
            <div className="space-y-4">
                <div className="p-4 bg-blue-50 dark:bg-slate-700/50 rounded-lg border border-blue-200 dark:border-slate-600">
                    ...
                    <ul className="list-disc list-inside mt-2 text-sm text-gray-700 dark:text-gray-300 space-y-1">
                        <li>O arquivo deve estar no formato CSV. A primeira linha deve ser o cabeçalho.</li>
                        <li>Colunas obrigatórias: <code className="bg-gray-200 dark:bg-slate-600 p-1 rounded">NOME</code>, <code className="bg-gray-200 dark:bg-slate-600 p-1 rounded">VALOR EMPRESTADO</code>, <code className="bg-gray-200 dark:bg-slate-600 p-1 rounded">TAXA JUROS %</code>, <code className="bg-gray-200 dark:bg-slate-600 p-1 rounded">DIA VENCIMENTO</code>.</li>
                         <li>A coluna `DIA VENCIMENTO` deve conter apenas o dia do mês (ex: 5, 15, 28).</li>
                        <li>Coluna opcional: <code className="bg-gray-200 dark:bg-slate-600 p-1 rounded">STATUS</code> (use "Atrasado" para marcar como inadimplente).</li>
                    </ul>
                    <div className="mt-3">
                                ...
                                <thead className="bg-gray-50 dark:bg-slate-700">
                                    <tr>
                                        {Object.keys(parsedData[0]).map(header => (
                                            <th key={header} className="px-4 py-2 text-left font-medium text-gray-500 dark:text-gray-300 capitalize">{header.replace(/_/g, ' ')}</th>
                                        ))}
                                    </tr>
                                </thead>
            ...
            <div className="mt-6 flex justify-end space-x-3">
                ...
                <Button type="button" onClick={handleSubmit} disabled={parsedData.length === 0 || !!error}>
                    Importar {parsedData.length > 0 ? parsedData.length : ''} Empréstimos
                </Button>
            </div>
        </Modal>
    );
